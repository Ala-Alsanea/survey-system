@php
    ...
    $getLogoClasses = fn(bool $isDarkMode): string => \Illuminate\Support\Arr::toCssClasses(['fi-logo', 'flex dark:hidden' => $hasDarkModeBrandLogo && !$isDarkMode, 'hidden dark:flex' => $hasDarkModeBrandLogo && $isDarkMode]);
...
@capture($content, $logo, $isDarkMode = false)
    ...
    @elseif (filled($logo))
        <img alt="{{ $brandName }}" loading="lazy" src="{{ $logo }}"
            {{ $attributes->class([$getLogoClasses($isDarkMode)])->style([$logoStyles]) }} />
    @else
        <div
            {{ $attributes->class([
