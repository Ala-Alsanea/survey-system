...
class Q10Chart extends ChartWidget
{
    protected static ?string $heading = 'Teacher has contract or appointment letter Chart';

    protected function getData(): array
    {
        $district = array_values(TeacherInfo::pluck('district')->unique()->all());
        // $answers = array_values(Survey::pluck('q_10')->unique()->all());
        ...
        $yes = array_values(array_map(fn ($val) => ['data' =>
        Survey::where('district', $val)
        ->where('q_10', 'like', '%نعم%')
        ->where('is_deleted', 1)
        ->count(),
        ...
        $no = array_values(array_map(fn ($val) => ['data' =>
        Survey::where('district', $val)
        ->where('q_10', 'like', '%لا%')
        ->where('is_deleted', 1)
        ->count(),
